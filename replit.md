...
- **Staff Directory:** Comprehensive staff management with CRUD operations, search, filtering (department, role, status), Excel import/export, and responsive table/card views. The staff form includes Role and Department dropdown fields that are dynamically populated from the Settings management section, ensuring consistency across the system. Includes a detailed employee profile view.
